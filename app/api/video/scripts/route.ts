...
import { buildVideoKeywordList } from '@/lib/services/video-keyword-source';
...
export async function GET(request: NextRequest) {
    try {
        ...
        const { data: job } = await supabaseAdmin
            .from('job_queue')
            .select('ats_keywords, buzzwords, hard_requirements, description, job_title, company_name')
            .eq('id', jobId)
            .eq('user_id', user.id)
            .single();
        ...
        const cleanKeywords = buildVideoKeywordList(job);

        return NextResponse.json({
            script: script || null,
            templates: templates || [],
            rawKeywords: cleanKeywords,
            job: {
