                        ...
                        try {
                            ...
                            if (lookupErr) {
                                ...
                                if (decision.shouldUpdate) {
                                    ...
                                    if (profileErr) {
                                        console.error(`[${requestId}] route=documents/upload step=save_profile supabase_error=${profileErr.message} — rolling back document + storage`);
                                        // Compensating action: rollback the document row and storage file
                                        // so we don't leave an orphan that blocks future uploads via the
                                        // Single-CV unique index.
                                        if (cvDocId) {
                                            await supabaseAdmin.from('documents').delete().eq('id', cvDocId);
                                        }
                                        await supabaseAdmin.storage.from('cvs').remove([cvUploadData.path]).catch(() => null);
                                        return NextResponse.json(
                                            {
                                                error: 'CV-Upload fehlgeschlagen — bitte erneut versuchen',
