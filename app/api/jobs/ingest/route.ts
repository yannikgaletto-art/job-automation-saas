...
export async function POST(request: NextRequest) {
  ...
  EXCLUDE: generic verbs ("Implementierung", "Schulungen", "Analyse"), language names ("Deutsch", "Englisch", "Fließend"), company/product names that are the job focus ("Odoo"-role = not a keyword), generic adjectives ("Agile", "modern", "Innovation"), soft skills ("Kommunikation"), JOB TITLES (acronyms like "SDR", "BDR", "KAM", "CEO/CFO/CTO/CMO" AND full titles like "Sales Development Representative", "Account Manager", "Project Manager" — Skill is "Project Management" or "Sales Development", NOT the title that ends in Manager/Representative/Executive/Director), DOMAIN/THEME terms (medical conditions like "Tinnitus", "AFib", "Vorhofflimmern", "COPD"; topic areas like "Healthcare", "Logistics" alone — only allow when paired with a specific tool/method like "SAP Healthcare"), FORMAT terms ("Webinare", "Workshops", "Seminare", "Konferenzen", "Trainings", "Schulungen") — these are delivery formats, not skills, VAGUE SOLO TERMS without skill content ("ICP", "Enrichment", "Playbooks", "Pipeline-Aufbau" — only keep when paired with a domain like "Lead Enrichment" or "Sales Playbooks").
