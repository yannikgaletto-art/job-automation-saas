...
|---------|----------------------|
| **Cleanup-Welle 2026-05-01 — Q5 Cert-Descriptions + Dead-Code + Test-Hygiene** | **Q5 Cert-Descriptions PROD-Bug behoben in 4 Layern: (1) `lib/services/cv-pdf-parser.ts` Mistral-Prompt: 9-Zeilen Cert-Description-Regel + Few-Shot durch eine Zeile ersetzt (`certifications[].description: alle Bullets unter dem Cert-Namen mit "\n" zu Plain-String joinen, NIE null wenn Bullets vorhanden`). (2) `types/cv.ts` Zod-Schema: fehlendes `description` Feld auf `certifications[]` ergänzt — vorher strippte `safeParse()` das Feld in confirm-parse. (3) `app/api/documents/reparse/route.ts` `maxDuration = 60` → `120` (Mistral OCR + Parse braucht 47–75s). (4) `components/cv-templates/ValleyTemplate.tsx`: hardcoded `MAX_CERTS = 6` Cap entfernt (droppte 3 von Yanniks 9 echten Certs). **Dead-Code-Removal (handover 1.5):** `handleOptimizeCV` + `<CVComparison>` Modal aus `app/[locale]/dashboard/job-queue/page.tsx` entfernt (dead path: `/api/cv/optimize` returnt jetzt diff-basierte Proposals, nicht den Legacy-`CVOptimizationResult`-Shape mit `.optimizedCV`/`.atsScore`/`.changesLog`). `components/cv/cv-comparison.tsx` + `components/cv/improvement-card.tsx` gelöscht; `CVOptimizationResult` Interface aus `types/cv.ts` raus; `onOptimize`/`optimizingJobId`/`isOptimizing` Props aus `JobQueueTable` + `JobRow` raus; orphan i18n keys (`review_cv`, `notify_cv_optimized`, `optimizing`) aus 3 Locales entfernt; `useCreditExhausted`-Import entfernt. **Test-Hygiene (handover 1.6):** `buildBlacklistPromptSection` (deprecated, delegierte nur an `buildLeanBlacklistSection`) komplett entfernt + Test angepasst auf neue Funktion. `app/api/inngest/__tests__/health.test.ts` hinter `RUN_LIVE=1` Env-Gate gestellt — der seit Wochen dokumentierte "1 pre-existing flake" verschwindet aus `npm test`. **Scratch-Files (handover 1.4):** 12 untracked Diagnose-Skripte aus `scripts/` gelöscht (alle `_diag-*.ts` + `_drop-yannik-live-cv.ts`). `scripts/_generate-synthetic-cv.ts` bleibt (E2E-Tests). **Q1/Q3/Q4 (handover):** Q1 done (0 docs auf allen Test-Accounts, cv-reset hat schon aufgeräumt); Q3 false-positive (`full_name=NULL` auf einem Account ist nur stale data von vor 2026-04-28 confirm-parse-Architektur, self-healing beim nächsten Confirm); Q4 disproved während prior session (cv_structured_data ist nicht NULL, Diag-Script hatte `.eq('user_id', ...)` statt `.eq('id', ...)` Bug). **Test-Gate:** 9/10 jest suites grün (1 skipped: inngest/health hinter RUN_LIVE), 66 passed 0 failed, `tsc --noEmit` clean, `npm run build` clean. **Folge-Welle offen:** Q2 Locale-Switch-Investigation, 1.3 Branch-Cleanup, 1.7 ARCHITECTURE.md/schema.sql/database-Doku-Sync.** |
| **Single-CV Migration — DB-Invariante "max 1 CV pro User" (2026-04-28)** | **Reduce-Complexity-Migration nach 3×8h Multi-CV-Bug-Wellen. Vor der Migration: 4 parallele Sync-Pfade (`documents`-Rows + `user_profiles.cv_*` + `job_queue.metadata.cv_snapshot` + 2-stufiger Resolver) erlaubten Drift, Vermischungen, Halluzinationen. Nach Migration: 1 CV pro User strukturell garantiert via partial UNIQUE INDEX `one_cv_per_user ON documents(user_id) WHERE document_type='cv'`. **11 Phasen:** (0) Audit-Skript `scripts/_audit-multi-cv-users.ts` listete 4 betroffene User (susan@subafilme.de + yannik.galetto@gmail.com + cgwkoch@aol.com + info@yannik-galetto.site, 7 Non-Master-Docs), 0 ambiguous → Plan-A safe. (0.5+1) `supabase/migrations/20260428_single_cv_invariant.sql`: backup-Tabellen `documents_backup_pre_singlecv` + `user_profiles_cv_backup_pre_singlecv` als Rollback-Pfad, defensive DELETE die nur Non-Master-CVs droppt wenn Master-Pointer auf existierendes Doc zeigt, partial UNIQUE INDEX (CONCURRENTLY skipped weil Tabelle klein + atomic-Rollback wichtiger), `cv_migration_seen_at` Spalte für Banner mit Backfill der Non-Impacted-User auf NOW() + DEFAULT NOW() für Future-Inserts. `scripts/_cleanup-orphan-cv-storage.ts` löscht orphan Storage-Files in Batches (Postgres kann Storage nicht erreichen). (2) Upload-Route `app/api/documents/upload/route.ts` 3-stufig gehärtet: Pre-Check `existingCvs.length > 0 → 409 CV_ALREADY_EXISTS` (friendlich, kein orphan storage), 23505-Handler im DB-INSERT-Catch (Race-Loser räumt Storage auf, 409), Compensating Action im profile-update (rollback documents-Row + Storage wenn user_profiles-Update failt). `app/api/documents/upload-simple/route.ts` GELÖSCHT (zero callers, hätte Invariante umgangen). (3) Delete-Route `app/api/documents/[id]/route.ts` reordert: Audit-Log → Auth → Profile-Clear (CV-only, idempotent) → DB-Delete → Storage best-effort. Verhindert phantom Master-Pointer bei DB-Failure nach Storage-Delete. (4) UI `components/profil/active-cv-card.tsx` (911→810 LOC, -101 Zeilen): CV-Hint-Dialog komplett raus (war Welle 1 "master kept on subsequent upload" — unter Single-CV unerreichbar weil 2nd Upload 409 zurückgibt), `master_action='kept'` Toast-Branch raus, Upload-Button cap 3→1, neuer Confirm-Delete-Dialog für CVs mit DSGVO-Pflicht-Hinweis, Bug-Fix: `delete_failed_toast` notify (war assigned aber nie surfaced). (5+7) Resolver-Robustheit-Tests in `lib/services/__tests__/job-cv-snapshot.test.ts` — 5 neue Cases dokumentieren Single-CV-Invariante: in-flight job survives master-delete via snapshot, snapshot wins über fresh master post-delete-then-re-upload, fresh job ohne snapshot+master returnt null cv. 15/15 grün. (8) Cover-Letter `setup-data/route.ts` refactored: 4-stufige inline-Logik durch `resolveJobCv` ersetzt für DRY-Konsistenz mit `cover-letter-generator.ts` (selbe SSoT). (11) Banner `components/profil/cv-migration-banner.tsx` + API `app/api/profile/dismiss-cv-migration/route.ts` + i18n-Namespace `cv_migration_banner` (de/en/es). Server-Page-Gate `(showMigrationBanner = withinOneDay && cv_migration_seen_at IS NULL)` — Banner erscheint nur 24h post-migration und nur für die 4 betroffenen User. **Test-Gate:** 601/602 jest grün (1 pre-existing inngest/health flake), `tsc --noEmit` clean, `npm run build` clean. **Forbidden Files unangetastet:** model-router.ts, middleware.ts, cv-match-pipeline.ts, alle bestehenden migrations (nur eine NEUE Migration angelegt). **Folge-Aktion (User auf Vercel-Preview):** (1) Migration applizieren via Supabase-CLI oder Dashboard. (2) `npx tsx scripts/_cleanup-orphan-cv-storage.ts` ausführen. (3) Smoke-Test: Profil → 2nd CV-Upload-Versuch zeigt Toast "Du hast bereits einen Lebenslauf...". (4) Trash-Button → Confirm-Dialog → Delete → Empty-State zeigt "Lade deinen Lebenslauf hoch...". (5) Re-Upload → einziger CV. (6) Banner sichtbar bei den 4 betroffenen Accounts.** |
| **CV Phase 8 — Bug-Trio: Name-Stop-Liste + Entity-Add-Halluzinations-Guard + Education-Grade-Strip (2026-04-27)** | **Tiefgreifende User-E2E-Diagnose nach Pre-Push-Test auf Localhost zeigte 3 kritische Bugs in Yannik's Optimizer-Output: (1) Title "Berlin Familienstatus" statt "Yannik Galetto"; (2) halluzinierte Experience-Station "KI-GTM-Beratung" die nirgends im Source-CV existiert; (3) Education-Doppel-Bullet "Abschlussnote: 1,3" sowohl als bold-Prefix als auch als description-Bullet. **Diagnose-Skripte:** scripts/debug-cv-source.ts zeigte Master-CV mit `full_name = "Berlin   Familienstatus"`, 5 Stationen mit Mishmasch (Innovation Manager Ingrano fehlt komplett, Co-Founder ohne Company). scripts/_fresh-parse-exxeta.ts + scripts/_fresh-parse-ai-ti.ts zeigten dass aktueller Parser "Berlin Familienstatus" als Tier-1-Name picked weil PERSON_NAME_RE matched (Berlin + Familienstatus = TitleCase + TitleCase, beide ≥3 chars) und INSTITUTIONAL_SUFFIX nicht greift. **Fixes:** **(A) Tier-1-Smart-Name-Stop-Liste erweitert** in `lib/services/cv-parser.ts:227+`: zwei neue Regex parallel zu INSTITUTIONAL_SUFFIX — `GERMAN_COMMON_NOUN` (familienstatus/familienstand/geburtsdatum/anschrift/telefon/email/nationalität/etc., 18 Tokens), `GERMAN_CITY` (berlin/münchen/hamburg/köln/frankfurt/stuttgart/etc., 40+ DACH-Cities) als first-token-Reject. Filter-Logik: candidate wird abgelehnt wenn (PERSON_NAME_RE || INSTITUTIONAL_SUFFIX || GERMAN_COMMON_NOUN match) ODER first-token in GERMAN_CITY. Verifikation: e2e-Test mit "Exxeta 04.08.1996 in Berlin   Familienstatus: ledig" header → "Yannik Galetto" wird korrekt gepicked statt "Berlin Familienstatus". **(B) `isProtectedEntityAdd` Guard** in `lib/services/cv-optimizer-sanitizer.ts:55+`: neue exportierte Pure Function flaggt `add`-Changes auf experience/education ohne `entityId` (= LLM versucht eine ganze NEUE Station/Bildungsstation zu erfinden). Bullet-level adds (mit entityId vorhanden) bleiben erlaubt — das sind legitime "missing achievement bullet"-Adds auf existierenden Stationen. Drop-Reason `protected_entity_add` neu in DropReason union; `sanitizeOptimizerChanges` wendet den Guard nach `isProtectedEntityRemove` an. **(C) `stripGradeFromEducationDescription` Post-Processor** in `cv-parser.ts:1138+`: scannt education[].description-Lines per `/^\s*[-•–*]?\s*(abschlussnote|note|grade|gpa|gesamtnote|durchschnittsnote)\s*:\s*([^\n]+?)\s*$/i`, droppt die Zeile aus description und extrahiert den Wert in education[].grade falls grade dort noch null. Idempotent. Zusätzlich: `recoverMissingEducationDescription` skipt jetzt grade-Lines via gleichem Regex (verhindert dass Bullets-Recovery Abschlussnote als ersten Bullet absorbiert). **Database-Heal:** scripts/_heal-master-cv.ts (3x ausgeführt) führt `syncMasterCvFromDocument({force:true})` auf Yannik's Exxeta-Doc — re-parst mit Phase-8-Code, Master ist jetzt clean (name=Yannik Galetto, 5 Stationen, correct dates, Education ohne Doppel-Bullet). **Tests:** 19 neue Cases gesamt: 9 stripGradeFromEducationDescription (REGRESSION Yannik M.Sc. + Idempotenz + grade-line-bare + EN-Grade/GPA + grade-existing-not-overwritten + multi-line-cleanup + null-description-fallback + preserve-other-fields + empty-array), 9 isProtectedEntityAdd (REGRESSION KI-GTM-Beratung + add-education-no-entityId + bullet-adds-allowed + skills/certs-adds-allowed + modify/remove-not-flagged + e2e-sanitize-drops + alle Sub-Cases), 3 e2e-cv-parser-Tests (Yannik-Berlin-Familienstatus-Repro + München-Geburtstag-Repro + Maria-Schmidt-still-works). 552/553 jest gesamt grün (1 pre-existing inngest/health flake), `tsc --noEmit` clean. **Limitations dokumentiert:** (a) Co-Founder ohne Company bleibt — Phase 5.1 fuzzy-recover fängt nicht alle Pattern-Varianten; future welle. (b) AI TI I CV.pdf hat fundamentales OCR-Problem (Datum-Spalten aggregiert) das Parser-immun ist — User soll Exxeta als Master nutzen. (c) Bild 3 zeigt "Innovation Manager Ingrano Solutions" als 1. Station, aber kein vorhandener CV-extracted_text enthält das verbatim → User muss diese Daten manuell pflegen oder neuen CV uploaden. **User-Verifikation auf Vercel-Preview:** (1) Refresh, Master-CV-Anzeige zeigt jetzt "Yannik Galetto" + 5 Stationen + clean Education; (2) Optimizer-Output halluziniert keine zusätzliche "KI-GTM-Beratung"-Station mehr; (3) Education-Render zeigt "Abschlussnote: 1,3" einmal als grade-Field, nicht als doppelter Bullet. Welle C Re-Parse-Button oder Re-Match auf neuen Job mit AI TI / CV I Yannik-Pick wenn er stattdessen jenen CV will.** |
| **CV Welle D + F — Valley→ATS-Label + Bullet-Description-Validator-Härtung (2026-04-27)** | **Zwei kleine Sammel-Wellen vor dem Push: **Welle D (Valley→ATS-Rename):** User-facing Template-Label `'Valley'` → `'ATS'` in `components/cv-optimizer/OptimizerWizard.tsx:545` (TEMPLATES-Konstante). Internal-ID bleibt `'valley'` für Backward-Compat (DB-Persistierung, switch-cases in PdfViewerWrapper/DownloadButton/download-route, CVOptSettings-Type). Class-Name `ValleyTemplate` bleibt unverändert (interner Codename, nicht User-facing). 1-Zeilen-Patch + Kommentar warum. Rationale: "ATS" beschreibt die Template-Funktion (ATS-optimiertes Layout) ehrlicher als Codename. **Welle F (Edge-Case #1: validateDescriptionsAgainstRawText Bullet-Härtung):** Education-Description-Recovery (Phase 5.2) schreibt Bullets MIT Prefix (`- Modul A\n- Modul B`); rawText hat sie ohne Prefix nur newline-getrennt. Pre-Welle-F droppte der Validator legitime Recoveries weil 5-Wort-Window-Match auf "konzeption qualitativer daten reduktion durch" suchte, aber rawText "konzeption qualitativer daten\nreduktion durch" hatte. **Fix:** neue helper `normaliseForMatch(s)` in `validateDescriptionsAgainstRawText` (`lib/services/cv-parser.ts:1102+`): toLowerCase + bullet-chars `[-•*–]` durch space ersetzt + `\s+` zu single-space kollabiert + trim. Wird auf rawText UND description angewendet, dann Substring-Match. Hallucinations-Drop bleibt funktional (Test verifiziert). **Tests:** 3 neue Cases in `lib/__tests__/cv-parser.test.ts` (Welle-F-Block): bullet-prefix-multi-line-match, in-line-dash-match, hallucinated-bulleted-still-drops. 135/135 cv-parser-Tests grün, 532/533 jest gesamt grün (1 pre-existing inngest/health flake), `tsc --noEmit` clean. **Akzeptierte Limitations (Welle F):** (a) i18n-Schuld auf hardcoded TEMPLATES-Labels nicht gefixt — separate i18n-Welle. (b) "Weitere Kurse (1,0)" Halluzinations-Bug aus Handover deferred — keine reproduzierbare Fixture. (c) PdfViewerWrapper i18n-Bruch deferred — separate UI-Welle.** |
| **CV-Optimizer Welle E — 2-3-Seiten-Garantie (Page-Mode-Toggle) (2026-04-27)** | **User-Wunsch: bei 6+ Stationen droppt der 2-Seiten-Modus zwingend Bullets bei alten Stationen (max 1 Bullet pro Old-Station — by design für Bewerbungs-Standard 2-Pager). User möchte Wahl-Option für 3-Seiten-Detailliert wenn er ältere Stationen ausführlicher zeigen will. **Implementation:** (1) Neuer Type `PageMode = '2-pages' | '3-pages'` in `types/cv-opt-settings.ts`; CVOptSettings-Interface erweitert um `pageMode: PageMode`; Default `'2-pages'` in `DEFAULT_CV_OPT_SETTINGS`. (2) Optimizer-Prompt `app/api/cv/optimize/route.ts`: vorher-Inline-LAYOUT-CONSTRAINTS-Block durch dynamische Variable `layoutConstraints` ersetzt — bei pageMode='3-pages' wird HARD RULE auf 3 Seiten + max 4 Bullets pro Entry + oldest 2 entries get max 3 bullets gesetzt (statt 1); SELF-JUDGE bullet-cap-thresholds entsprechend angepasst (>24 statt >15). (3) ValleyTemplate.tsx: neue Konstante `MAX_BULLETS_3PAGES=4`, Component-Prop `pageMode?: '2-pages' \| '3-pages'`, maxBullets-Logik: `pageMode === '3-pages' ? 4 : layoutMode === 'compact' ? 3 : 3`. (4) TechTemplate.tsx: gleicher pageMode-Prop, hardcoded `slice(0, 4)` ersetzt durch `slice(0, maxBullets)` mit `maxBullets = pageMode === '3-pages' ? 4 : 3`. (5) PdfViewerWrapper.tsx + DownloadButton.tsx: pageMode-Prop durchreichen via resolveTemplate / resolveDocument; useEffect-Deps um pageMode erweitert (sonst kein Re-Render bei Mode-Switch). (6) OptimizerWizard.tsx: Toggle-UI in `display_options`-Details-Block nach Languages-Toggle: 2 Radio-Buttons mit Beschreibung "2 Seiten ist Bewerbungs-Standard. 3 Seiten erhält mehr Detail bei älteren Stationen." Pass-Through an PdfViewer + DownloadButton + Optimize-API-Body (`cv_opt_settings.pageMode`). (7) i18n in 3 Locales (de/en/es) Namespace `cv_optimizer`: 4 neue Keys (`page_mode_label`, `page_mode_desc`, `page_mode_2`, `page_mode_3`). (8) **Vercel-Härtung:** `maxDuration` von 120s auf 150s erhöht — pageMode='3-pages' addiert ~30% Output-Tokens, alte 120s-Buffer wäre scharf bei 6+ Stationen. **Tests:** keine neuen — pageMode-Flow ist UI-Toggle + Prompt-String-Concat + Template-Prop-Drilling, Manual-Verifikation Pflicht. 529/530 jest grün (1 pre-existing inngest/health flake), `tsc --noEmit` clean. **Akzeptierte Limitations:** (a) MAX_CHANGES=18 bleibt unverändert; bei 6 Stationen × 4 Bullets könnte cap getroffen werden, priority-sort (modify > add > remove) erhält die wichtigsten. (b) pageMode wird nicht in DB persistiert (ist client-state in CVOptSettings) — User wählt jedes Mal neu. **User-Verifikation:** Optimizer öffnen → "Anzeigeoptionen" expanden → "Seitenanzahl" Toggle → "3 Seiten" → Optimieren → PDF zeigt mehr Bullets pro Station, auch bei alten. Default '2-pages' = identisches Verhalten zu vorher.** |
