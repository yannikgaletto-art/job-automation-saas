...
| **CL Phase 5 Hardening (2026-04-24)** | **Regex-Post-Gen-Validator um 9 Patterns erweitert: Kontrast "nicht X, sondern Y", Komparativ-Selbstlob, halluzinierte Studien, 3rd-Person-Firma, ICH-zentrierte Brücke, Unternehmens-Apposition, "[Autor] meinte damit", anthropomorphe Firma ("X hat mich gelehrt"), "war mir sofort klar". Pathly-DNA-Toggle fuer Custom-Style (default OFF). Prompt-Haertung: KEIN-ZITAT wenn kein Quote, FIRMEN-ANSPRACHE (2nd-Person-Pflicht), AUTOR-ICH-PERSPEKTIVE, REFERENZBRIEF-KALIBRIERUNG-Checkliste. activeTone bei Custom-Style gemutet. hasQuote via .quote?.trim() gehaertet. Upload-Limit-Fix: generierte Drafts (origin=generated) zaehlen nicht mehr. Wizard-Escape: User kann ohne Hook fortfahren bei leerer Perplexity-Analyse. Conflict-Guard: Custom-Style + DNA-OFF verwirft vor-gewaehltes Zitat inline. multi-agent-pipeline + kill-fluff archiviert (0 Caller). rhetorical_contrast_pattern Feld aus StyleAnalysis entfernt (war Konflikt-Quelle). Handover-MD fuer Wizard-Step-0-Refactor.** |
| **CL Phase 5.3 Sync-Loop Fix (2026-04-24)** | **Kritischer Architektur-Bug behoben: scanForFluff lief NACH dem Break-Check, wurde bei Judge-PASS umgangen. Folge: "kein X, sondern Y" rutschte durch obwohl Regex korrekt. Fix (cover-letter-generator.ts): scanForFluff ist jetzt Break-Blocker (judgePassed && !iterFluff.found). Blacklist um 3 weitere Patterns erweitert. Klammer-Technik ist jetzt DNA-Pflicht auch OHNE Zitat. Base: 60 Tests gruen.** |
| **Profil-Tab Phase 1 (2026-04-25)** | **Neuer Dashboard-Tab /dashboard/profil fuer Dokument-Verwaltung. ActiveCVCard nach components/profil/ verschoben; Sidebar-NavItem mit User-Icon; i18n-Keys (dashboard.nav.profile). Redirect-Vertrag-Migration: 5 Caller auf /dashboard/profil umgezogen. Settings-Documents-Card entfernt. ActiveCVCard i18n-Schuld behoben. CL Custom-Style Click triggert DocumentsRequiredDialog.** |
| **Profil-Tab Phase 1.1 - Launch-Waitlist Migration (2026-04-25)** | **Launch-Waitlist-Card aus Settings nach Profil migriert. i18n-Namespace settings.launch_waitlist nach profil.launch_waitlist umgezogen (15 Keys x 3 Locales). Profil-Page rendert 2 Cards: Documents + Launch-Waitlist.** |
